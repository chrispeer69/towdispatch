...
```
s3://towcommand-backups/postgres/
...
`s3://towcommand-tenants/<tenant-id>/`:
...
- **Object lock: not set** on tenant uploads (would prevent legit deletes); **on** for `s3://towcommand-incidents/` (forensic captures, 7-year object lock)
- **Lifecycle rules** to enforce retentions above

### Backup of S3

S3 itself is 11-nines durable; no separate backup. **Cross-region replication is Phase 1** (`towcommand-tenants` → `towcommand-tenants-dr` in `us-west-2`) for disaster recovery on the bucket level. RPO for tenant uploads is "what S3 says it is" plus the replication lag.
...
2. `s3://towcommand-backups` bucket with the lifecycle rules.
3. Cross-region replication for `s3://towcommand-tenants`.
