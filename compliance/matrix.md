...
| [cc6-logical-access.md](controls/cc6-logical-access.md) | CC6 | JWT auth, argon2id, TOTP MFA (login-gate enforcement via `MFA_LOGIN_GATE_ENABLED`, default off — production value is part of the evidence), RLS tenant isolation, RBAC | User/role CSV, MFA enrollment, `MFA_LOGIN_GATE_ENABLED` prod value, RLS test | `list-users-roles.ts`, `list-admins.ts` | CTO |
