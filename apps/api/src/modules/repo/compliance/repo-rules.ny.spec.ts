/**
 * Repo rule-engine spec — NY (UCC + Banking Law 108, 15-day redemption, secondary contact).
 */
...
const R = REPO_STATE_RULES.NY;
const ST = 'NY' as const;

describe('repo rules — NY', () => {
  it('config matches the NY statute profile', () => {
    expect(R.postRepoNoticeDays).toBe(10);
    expect(R.redemptionPeriodDays).toBe(15);
    expect(R.cureRightDays).toBe(15);
    expect(R.personalPropertyHoldDays).toBe(45);
  ...
  it('holds personal property for 45 days', () => {
    const hold = computePersonalPropertyHold(OPENED, R);
    expect(hold.holdDays).toBe(45);
    expect(hold.holdUntil).toBe(addUtcDays(OPENED, 45).toISOString());
