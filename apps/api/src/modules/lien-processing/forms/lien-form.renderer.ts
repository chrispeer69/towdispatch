...
/**
 * The salient textual content of a lien notice, independent of PDF layout.
 * Pulled out as a pure builder so the content (statute citation, case id,
 * title, body) is unit-testable without decompressing the PDF stream — PDFKit
 * deflates text streams by default, so the rendered Buffer is not greppable.
 * `draw()` sources its strings from this so the document and the assertions
 * can never drift.
 */
export interface LienNoticeContent {
  caseId: string;
  state: string;
  statute: string;
  title: string;
...
export function buildLienNoticeContent(input: RenderLienFormInput): LienNoticeContent {
  ...
  return {
    ...
    recipientHeading: isPublication ? 'TO:' : 'TO THE REGISTERED OWNER / LIENHOLDER:',
    noticeBody: `You are hereby notified that the above-described vehicle is being held by ${input.tenantName} and is subject to a lien for towing, storage, and related charges under ${input.rules.statute}. Unless the accrued charges are paid in full and the vehicle is reclaimed, the vehicle will be sold at a public lien sale to satisfy the lien.`,
    earliestSaleDate: formatDay(earliestSale.toISOString()),
...
export class LienFormPdfService {
  ...
  private draw(doc: PDFKit.PDFDocument, input: RenderLienFormInput): void {
    ...
    this.kv(doc, 'Sale will NOT occur before', content.earliestSaleDate);
