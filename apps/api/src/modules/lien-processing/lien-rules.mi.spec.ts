/**
 * Lien rule-engine spec — Michigan (MCL 257.252 / 570.521-.530).
 * dmv 7 · owner/lienholder wait 20 · publication +10 (low-value exempt) ·
 * min 30.
 */
...
const R = LIEN_STATE_RULES.MI;
const ST = 'MI' as const;

describe('lien rules — MI', () => {
  it('buckets value by the MI thresholds', () => {
  ...
  it('earliest sale honors the longer 20-day notice wait when publication is early', () => {
    const facts = makeFacts(ST, {
      currentStep: 'waiting_period',
      lienholderFound: false,
      ownerNoticeSentAt: addUtcDays(OPENED, 25),
      publicationCompletedAt: addUtcDays(OPENED, 26),
    });
    // max(opened+30, ownerSent+20=+45, pub+10=+36) = +45
    expect(computeEarliestSaleDate(facts, R).toISOString()).toBe(
      addUtcDays(OPENED, 45).toISOString(),
    );
  });
