/**
 * Lien rule-engine spec — Illinois (625 ILCS 5/4-201..214 / 770 ILCS 50).
...
const R = LIEN_STATE_RULES.IL;
const ST = 'IL' as const;

describe('lien rules — IL', () => {
  it('buckets value by the IL thresholds', () => {
  ...
  it('blocks on a recorded claim', () => {
    const next = computeNextAction(
      makeFacts(ST, { currentStep: 'publication_complete', lienholderResponseAt: OPENED }),
      R,
      OPENED,
    );
