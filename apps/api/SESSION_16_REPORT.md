...
**Date:** 2026-05-12
**Branch:** `master`
**Status:** Shipped. The importer migrates 5+ years of historical Towbook data into TowCommand Pro for Roadside Towing and Recovery, Inc. (tenant #001) and Auto Lyft (tenant #002) once the founder produces an export bundle.
...
5. Download the `.zip`. Do **not** unzip it — TowCommand consumes the ZIP directly.
...
1. Log in to TowCommand as the **owner** of the target tenant.
...
4. Look at each card:
   - **Missing** = `0` → every row in the bundle made it into TowCommand.
   - **Orphaned** = `0` → no TowCommand rows are stamped with `external_source='towbook'` that aren't in the bundle (would suggest you imported, then someone deleted things in Towbook).
