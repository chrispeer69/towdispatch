...
journal entry's `idx`/`when` are increasing, so `pnpm --filter @towcommand/db
...
Existing tests: **no regressions** — `pnpm --filter @towcommand/api test`
...
- **Migrations to run on next deploy**:
  1. `pnpm --filter @towcommand/db migrate` — applies
