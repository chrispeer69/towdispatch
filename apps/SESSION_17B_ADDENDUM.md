...
$ pnpm --filter @towcommand/api test        ✓ 138 passed, 18 DB-gated skips
$ pnpm --filter @towcommand/web build       ✓ green
$ pnpm --filter @towcommand/web typecheck   ✓ zero errors
$ pnpm --filter @towcommand/e2e typecheck   ✓ zero errors
$ pnpm --filter @towcommand/e2e test        ✓ 13 register, all stack-gated
```
...
- New steps: build the api, build the web, install Playwright Chromium, start API as background process on port 3601, start web (`next start`) on port 3000, poll `/health` and `/login` until both respond, then run `pnpm --filter @towcommand/e2e test` which now includes Lighthouse by default.
