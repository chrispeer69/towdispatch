...
export function ImpoundYardArticle(): JSX.Element {
  return (
    <div className="pb-12">
      <P>
        The Impound & Storage Yard module is currently in active development (Phase 2). This guide
        outlines the capabilities that will be available when the module launches.
      </P>
      <P>
      ...
      <P>
        Track vehicles across multiple physical storage lots. You will be able to define yards,
        zones, and specific stalls. When a driver drops a vehicle, they log its exact location,
        eliminating the "walking the yard" search.
      </P>

      <H3>2. Automated Daily Storage Accruals</H3>
      <P>
        Instead of manually calculating how many days a vehicle has been on the lot when the owner
        arrives to claim it, the system runs a midnight cron job that automatically posts the daily
        storage fee to the vehicle's running tab, respecting the specific rate sheet for that yard
        and vehicle class.
      </P>

      <H3>3. Hold Types and Legal Clocks</H3>
      <P>
        Different impounds have different legal rules. The system will track the specific "Hold
        Type" (e.g., Police Hold, Abandoned Vehicle, Repossession, Motor Club Wait). If a vehicle is
        on a Police Hold, the system prevents it from being released or advancing down the lien
        pipeline until the hold is officially lifted in the system.
      </P>

      <H3>4. The Lien Notice Ladder</H3>
      <P>
        This is the core moat of the module. Every state has strict statutory requirements for when
        and how you must notify a registered owner before you can auction their vehicle.
      </P>
      <UnorderedList>
        <li>The system tracks the exact hour the vehicle entered the yard.</li>
        <li>
          It automatically generates the 7-day, 14-day, or 30-day notice letters formatted to your
          specific state's legal requirements.
        </li>
        <li>It tracks certified mail tracking numbers to prove compliance.</li>
        <li>
          Once the statutory waiting period expires and all notices are sent, the vehicle is
          automatically flagged as "Lien Eligible."
        </li>
      </UnorderedList>

      <H3>5. Auction Preparation</H3>
      <P>
        Vehicles that clear the lien process are grouped into Auction Batches. The system generates
        the necessary paperwork (e.g., Ohio BMV forms) and prepares the data export required by
        major auction houses like Copart or IAA.
      </P>

      <Callout tone="info" title="Early Access">
        We are currently interviewing operators in Ohio, Florida, and Texas to ensure the
      ...
      <div className="space-y-4 mt-6">
        <div>
          <strong className="block text-text-primary-on-dark">
            Can I use the current system for impounds?
          </strong>
          <p className="mt-1 text-sm text-text-secondary-on-dark">
            Yes, but manually. You can create a job with the Service Type <Code>Impound</Code>, drop
            the vehicle at your yard address, and manually add line items for storage days when you
            generate the invoice. The automated daily accruals and lien tracking will replace this
            manual process when the module launches.
          </p>
        </div>
      </div>
    </div>
