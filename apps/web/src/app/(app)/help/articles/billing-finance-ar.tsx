...
export function ArAgingArticle(): JSX.Element {
  return (
    <div className="pb-12">
      <P>
        Cash flow leak is a silent killer in the towing industry. If your accounting team is relying
        on spreadsheets or memory to track which motor club owes you for last month's tows, you are
        losing money. The <Em>A/R Aging Workspace</Em> replaces those spreadsheets with a real-time,
        actionable dashboard.
      </P>
      <P>
        This guide explains how to use the A/R workspace to identify past-due invoices, filter by
        specific accounts, and send bulk reminder emails with one click.
      </P>

      <H2>Overview</H2>
      <P>
        The A/R (Accounts Receivable) Aging Workspace is located under{' '}
        <Em>Billing → A/R Workspace</Em>. It displays every posted invoice in your system that has
        not been fully paid.
      </P>
      <P>
        Unlike a static report, this workspace is interactive. You can filter the list to show only
        invoices older than 30 days, select them all, and click "Send Reminder" to instantly email
        the customers. The system automatically calculates whether an invoice is "Past Due" based on
        the delinquency threshold you set for that specific account.
      </P>

      <Callout tone="tip" title="What defines 'Past Due'?">
        "Overdue" and "Past Due" are not the same thing. An invoice might be overdue by 1 day, but
        if your contract with Agero gives them 30 days to pay, it isn't "Past Due" (delinquent)
        until day 31. The A/R Workspace respects the <Em>Delinquency Days Threshold</Em> you
        configure for each Account in <Em>Settings → Accounts</Em>. If you haven't set a threshold,
        it uses the global default from <Em>Settings → Invoice Defaults</Em>.
      </Callout>

      <H2>When to use what</H2>
      <P>The workspace provides several tools to help you manage your collections workflow.</P>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <div className="rounded-[10px] border border-divider bg-bg-surface-elevated/20 p-5">
          ...
          <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-text-secondary-on-dark">
            <li>
              Filter by Status (e.g., show only <Code>Past Due</Code>).
            </li>
        ...
        <div className="rounded-[10px] border border-divider bg-bg-surface-elevated/20 p-5">
          ...
          <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-text-secondary-on-dark">
            ...
            <li>
              Click <Code>Send reminder email</Code> to re-send the invoice with a "Past Due"
              notice.
            </li>
            <li>
              Generates statements for a single account (requires filtering by Account first).
            </li>
          </ul>
        </div>
      </div>
      ...
      <P>
        It's the 15th of the month. Your accounting manager needs to follow up with AAA regarding
        unpaid invoices from two months ago.
      </P>
      ...
      <OrderedList>
        ...
        <li>
          Click the <Code>Past Due</Code> status chip to ensure you are only looking at delinquent
          invoices.
        </li>
        <li>
          Click <Code>Apply</Code>. The table updates to show only the AAA invoices that have
          crossed your configured delinquency threshold.
        </li>
      </OrderedList>
      ...
      <OrderedList>
        <li>
          Look at the sticky summary footer at the bottom of the screen. It calculates the totals
          for the currently filtered view.
        </li>
        <li>You see that AAA has 14 past-due invoices totaling $2,450.00.</li>
      </OrderedList>

      <H3>Step 3: Send Bulk Reminders</H3>
      <OrderedList>
        <li>Click the master checkbox at the top left of the table to select all 14 rows.</li>
        <li>
          The Bulk Action bar appears at the top of the table. Click{' '}
          <Code>Send reminder email</Code>.
        </li>
        <li>
          The system automatically emails the AAA billing contact with copies of all 14 invoices,
          flagged as past due.
        </li>
      </OrderedList>

      <H2>Troubleshooting Tree</H2>
      <P>If the A/R Workspace isn't showing what you expect, check these common issues.</P>
      ...
      <div className="mt-6 space-y-4">
        <div className="rounded-[10px] border border-divider bg-bg-surface p-5">
          ...
          <UnorderedList>
            <li>
              <Em>Check the Account's delinquency threshold.</Em> Navigate to{' '}
              <Em>Settings → Accounts</Em>, open the account, and check the "Delinquency Days"
              setting. If it's set to 45 days, an invoice won't flag as Past Due until day 46.
            </li>
            <li>
              <Em>Check the global default.</Em> If the account has no specific threshold, the
              system uses the default in <Em>Settings → Invoice Defaults</Em>.
            </li>
          </UnorderedList>
        </div>
        ...
        <div className="rounded-[10px] border border-divider bg-bg-surface p-5">
          ...
          <UnorderedList>
            <li>
              <Em>Filter by a single account first.</Em> You cannot generate a consolidated
              statement across multiple different customers or motor clubs. You must use the Filter
              Bar to select exactly one Account before the button becomes active.
            </li>
          </UnorderedList>
        </div>
      </div>
      ...
      <div className="space-y-4 mt-6">
        <div>
          <strong className="block text-text-primary-on-dark">
            Can I export this list to Excel?
          </strong>
          <p className="mt-1 text-sm text-text-secondary-on-dark">
            Yes. Click the <Code>Reports →</Code> button in the top right, or use the{' '}
            <Code>Excel</Code> button in the filter bar, to download a full CSV/Excel export of your
            A/R aging data for use in pivot tables or external reporting.
          </p>
        </div>
        <div>
