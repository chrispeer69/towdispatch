/**
 * Catch-all BFF for /api/jobs/* — proxies to the API with refresh-on-401.
 * Mirrors the Session 10 billing BFF.
 *
 * Powers the Invoice Review screen's "+ Add driver" action against
 * POST /jobs/:id/drivers and any future job-side endpoints.
 */
...
async function proxy(
  ...
  try {
    const data = await apiServerBff<unknown>(`/jobs/${tail}${search}`, {
      method,
      ...(body !== undefined ? { body } : {}),
    });
