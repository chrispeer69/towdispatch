/**
 * Catch-all BFF for /api/notifications/* — proxies to the API with
 * refresh-on-401. Mirrors the billing / fleet BFFs. Used by the notification
 * center, /notifications, /settings/notifications, and the admin views.
 */
...
async function proxy(
  ...
  try {
    const data = await apiServerBff<unknown>(`/notifications/${tail}${search}`, {
      method,
      ...(body !== undefined ? { body } : {}),
    });
