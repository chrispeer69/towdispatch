/**
 * Catch-all BFF for /api/payments/* — proxies to the API with refresh-on-401.
 * Mirrors the Session 10 billing BFF.
 *
 * Webhook ingest (POST /webhooks/stripe) is NOT routed through this BFF —
 * Stripe posts directly to the API origin so the request body matches the
...
async function proxy(
  ...
  try {
    const data = await apiServerBff<unknown>(`/payments/${tail}${search}`, {
      method,
      ...(body !== undefined ? { body } : {}),
    });
