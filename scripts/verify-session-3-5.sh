...
[ -f /c/dev/towdispatch/apps/api/src/integrations/ecosystem/ecosystem-provider.interface.ts ] \
  && echo "ECOSYSTEM_FILE=PASS" || echo "ECOSYSTEM_FILE=FAIL"
grep -q "EcosystemPartner" /c/dev/towdispatch/apps/api/src/integrations/ecosystem/ecosystem-provider.interface.ts \
  && echo "ECOSYSTEM_IFACE=PASS" || echo "ECOSYSTEM_IFACE=FAIL"
grep -q "CONVINI" /c/dev/towdispatch/apps/api/src/integrations/ecosystem/ecosystem-provider.interface.ts \
  && echo "ECOSYSTEM_COMMENT=PASS" || echo "ECOSYSTEM_COMMENT=FAIL"

# ---------- no UI references motor_club_member as a customer type ----------
if grep -rn "motor_club_member" /c/dev/towdispatch/apps/web/src 2>/dev/null; then
  echo "UI_NO_MCM=FAIL"
else
  echo "UI_NO_MCM=PASS"
fi
