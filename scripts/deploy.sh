...
run pnpm --filter @towcommand/api build

echo "[deploy] building web"
run pnpm --filter @towcommand/web build
...
if [[ "${SKIP_TESTS:-0}" != "1" ]]; then
  ...
  run pnpm --filter @towcommand/api test
  run pnpm --filter @towcommand/api typecheck
  run pnpm --filter @towcommand/web typecheck
  run pnpm --filter @towcommand/e2e typecheck
fi
...
run pnpm --filter @towcommand/db migrate
...
case "$DEPLOY_PLATFORM" in
  ...
  aws)
    ...
    echo "DRY-RUN aws ecs update-service --cluster towcommand-$ENV_TARGET --service api --force-new-deployment"
    echo "DRY-RUN aws ecs update-service --cluster towcommand-$ENV_TARGET --service web --force-new-deployment"
    ;;
  *)
...
case "$ENV_TARGET" in
  production) BASE='https://api.towcommand.com' ;;
  staging)    BASE='https://api-staging.towcommand.com' ;;
  *)          BASE='https://api-dev.towcommand.com' ;;
esac
